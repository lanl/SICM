#ifndef SICM_DEVICE_CXX_11_ALLOCATOR
#define SICM_DEVICE_CXX_11_ALLOCATOR
...
//     template <class U> struct rebind {typedef allocator<U> other;};
    ...
    SICMDeviceAllocator(sicm_device *dev = sicm_default_device(-1)) noexcept
        : sicm_dev(dev)
    {}

    template <class U> SICMDeviceAllocator(SICMDeviceAllocator<U> const& u) noexcept
        : sicm_dev(u.sicm_dev)
    {}
...
bool
operator==(SICMDeviceAllocator<T> const&, SICMDeviceAllocator<U> const&) noexcept
{
    return true;
}
...
template <class T, class U>
bool
operator!=(SICMDeviceAllocator<T> const& x, SICMDeviceAllocator<U> const& y) noexcept
{
    return !(x == y);
}

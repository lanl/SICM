...
void SiteReadsAgg_init(SiteReadsAgg * sra) {
    sra->histograms = internal_calloc(num_static_sites + 1, sizeof(hist_t));
}
...
void ThreadReadsInfo_init(ThreadReadsInfo * tri) {
    tri->top = tri->list = internal_malloc(sizeof(AddrTicks) * list_flush_limit);
    tri->histograms      = internal_calloc(num_static_sites + 1, sizeof(hist_t));
}
...
void ThreadReadsInfo_finish(ThreadReadsInfo * tri) {
    ...
    for (i = 0; i < max_threads; i += 1) {
        if (tris[i] == tri) {
            ...
            internal_free(tri->list);
            internal_free(tri->histograms);
            internal_free(tri);
            tris[i] = NULL;
            return;
        }
...
static ThreadReadsInfo * get_tri() {
    ...
    if (tri == NULL) {
        ...
        idx = n_tris++;
        tri = (ThreadReadsInfo*)internal_malloc(sizeof(ThreadReadsInfo));
...
void sh_rdspy_init(int _max_threads, int _num_static_sites) {
    ...
    tris = internal_malloc(sizeof(ThreadReadsInfo*) * max_threads);

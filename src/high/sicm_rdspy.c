...
void SiteReadsAgg_init(SiteReadsAgg * sra) {
    sra->histograms = __libc_calloc(num_static_sites + 1, sizeof(hist_t));
}
...
void ThreadReadsInfo_init(ThreadReadsInfo * tri) {
    tri->top = tri->list = __libc_malloc(sizeof(AddrTicks) * list_flush_limit);
    tri->histograms      = __libc_calloc(num_static_sites + 1, sizeof(hist_t));
}
...
static ThreadReadsInfo * get_tri() {
    ...
    if (tri == NULL) {
        ...
        idx = n_tris++;
        tri = (ThreadReadsInfo*)__libc_malloc(sizeof(ThreadReadsInfo));
...
void sh_rdspy_init(int _max_threads, int _num_static_sites) {
    ...
    tris = __libc_malloc(sizeof(ThreadReadsInfo*) * max_threads);

...
void *create_profile_arena(int index) {
  ...
  profinfo = __libc_calloc(1, sizeof(profile_info));
...
void setup_profile_thread(void *(*main)(void *), /* Spinning loop function */
  ...
  prof.profile_threads = __libc_realloc(prof.profile_threads, sizeof(profile_thread) * prof.num_profile_threads);
...
void initialize_profiling() {
  ...
  prof.info = __libc_calloc(tracker.max_arenas, sizeof(profile_info *));
